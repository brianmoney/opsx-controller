...
14. Move the change into `openspec/changes/archive/YYYY-MM-DD-<change>`. This
...
16. Follow the repo archive instructions in `AGENTS.md`, staging only the rest
    of the explicit archive set: synced `openspec/specs/` files, and the
    implementation files from step 6. Leave the change-directory deletion from
    step 15 staged; do not unstage it. Never stage
...
18. Create the required archive commit with the exact message
    ...
    string; the move still makes this a success.
...
- Return `status=archived` only when this run completed the archive move, and
  either created the required archive commit or confirmed nothing needed to be
  staged. Never reuse an existing archive directory, prior commit at `HEAD`,
  or state/history evidence as a success proxy.
