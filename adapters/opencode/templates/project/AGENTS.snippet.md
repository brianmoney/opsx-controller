...
convention: `.opencode/opsx-controller/plan-authoring.md` (project-first)
or `~/.config/opencode/opsx-controller/plan-authoring.md` (global fallback).
...
- `opsx-run` — manual single-change implement-review-archive loop. Supports
  exactly one change per run.

Durable controller state lives under `.opencode/opsx-controller/<change-id>.json`.
...
markdown document. The command reads the shared reference, validates the
request, delegates to the authoring agent, and reports whether compilation
self-checked. Detailed plan conventions (structure, dependency rules,
compile semantics) live in the plan-authoring reference, not in the command
body.
