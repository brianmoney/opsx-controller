...
- project scope: `.codex/opsx-controller/README.md`
- global scope: `~/.codex/opsx-controller/README.md`
...
- `<change-id>.json` files are written to `.opsx-controller/` at the project
  root (not under `.codex/`) because Codex sandbox protects the `.codex/`
  directory from agent writes even in `workspace-write` mode.
...
- After changing `.codex/agents/`, restart Codex CLI so updated agents are
  loaded.
