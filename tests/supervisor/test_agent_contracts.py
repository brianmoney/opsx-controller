...
class EgressEnforcementTests(AgentContractTestCase):
    ...
    def test_bridge_refuses_an_arbitrary_loopback_transport_target(self) -> None:
        """A loopback target that is not the launched server is not isolation."""
    ...
    def test_bridge_refuses_and_fails_the_action_when_unenforced(self) -> None:
        ...
        class _Transport:
            address = "10.0.0.5:4090"

        bridge.bridge.transport = _Transport()
        original = bridge.bridge.prompt_async
        bridge.bridge.prompt_async = lambda *a, **k: action_calls.append("prompt")
        try:
            with self.assertRaises(contracts_mod.EgressEnforcementError):
    ...
    def test_bridge_create_session_binds_the_roles_concrete_agent(self) -> None:
        ...
        with FakeOpencodeServer() as server:
            ...
            try:
                created = bridge.create_session(title="fixer-session", role="fixer")
            finally:
                bridge.bridge.create_session = original
        self.assertTrue(created.get("id"))
        self.assertEqual(captured, [{"title": "fixer-session", "agent": "opsx-fixer", "model": None}])
...
class ShellBypassPreventionTests(unittest.TestCase):
    """A shell bypass into a model client/agent runner is refused and journaled."""

    def test_a_model_client_invocation_is_refused(self) -> None:
        for command in ("opencode run", "codex exec", "claude -p hi"):
            with self.subTest(command=command):
                decision = worker_exec_mod.classify_command(command)
                self.assertFalse(decision["allowed"])
                self.assertEqual(decision["kind"], "unjournaled_model_client")

    def test_a_model_client_reached_through_indirection_is_refused(self) -> None:
        for command in ("$(opencode run)", "echo `opencode run`", "eval opencode run"):
            with self.subTest(command=command):
                decision = worker_exec_mod.classify_command(command)
                self.assertFalse(decision["allowed"], command)

    def test_a_script_interpreter_is_refused(self) -> None:
        for command in ("eval opencode run", "source env.sh", ". env.sh"):
            with self.subTest(command=command):
                decision = worker_exec_mod.classify_command(command)
                self.assertFalse(decision["allowed"], command)
    ...
    def test_a_nested_shell_is_refused(self) -> None:
        decision = worker_exec_mod.classify_command(["bash", "-c", "true"])
        self.assertFalse(decision["allowed"])
        self.assertEqual(decision["kind"], "nested_shell")

    def test_a_model_client_named_after_a_launcher_is_refused(self) -> None:
        decision = worker_exec_mod.classify_command("env FOO=bar opencode run")
        self.assertFalse(decision["allowed"])
        self.assertEqual(decision["kind"], "unjournaled_model_client")

    def test_an_ordinary_check_is_allowed(self) -> None:
        for command in ("python3 -m unittest tests", "node tests/x.js", "git status"):
            with self.subTest(command=command):
                decision = worker_exec_mod.classify_command(command)
                self.assertTrue(decision["allowed"], decision)

    def test_a_refusal_is_reported_to_the_worker_endpoint(self) -> None:
        reported: list[dict] = []

        def fake_dispatch(verb, **kwargs):
        ...
        code = worker_exec_mod.main(
            ...
            env={
                ...
                "OPSX_SUPERVISOR_ROLE": "fixer",
                "OPSX_SUPERVISOR_AGENT": "opsx-fixer",
        ...
        self.assertEqual(reported[0]["role"], "fixer")
