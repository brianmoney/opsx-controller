...
4. **Use `null` for unavailable usage, `0` for zero usage, with a `usage_available` boolean for fast checks.** Token fields (`input_tokens`, `output_tokens`, `cached_input_tokens`, `reasoning_tokens`, `total_tokens`) are nullable integers. A `null` value means the provider did not report usage (or extraction failed). A `0` value means the provider reported zero usage for that category. The `usage_available` boolean is `true` when at least one token field has a non-null value and `false` when all are `null`, giving consumers a single-field check before iterating token counts. A companion `usage_source` field records how usage was obtained (e.g. `"worker_json"`, `"log_metadata"`, `"provider_api"`) and is `null` when `usage_available` is `false`.
   Rationale: Reports and dashboards must distinguish "unknown cost" from "zero cost" to avoid misleading conclusions. Null-vs-zero encodes this at the field level, while `usage_available` gives consumers a fast "is there any usage data at all?" check without inspecting every token field. `usage_source` preserves provenance so downstream tools can assess reliability of the numbers.
   Alternatives considered: Relying solely on null checking without `usage_available` (requires inspecting every field, complicates partial-usage cases), negative sentinel values (violates JSON type expectations).
