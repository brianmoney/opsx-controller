...
The shared runtime installation SHALL replace its managed runtime libraries and
executables on repeated global installs. `opsx-plan doctor` SHALL continue to
detect an installed executable whose content differs from the repository copy,
independent of which adapter installer last deployed it.
